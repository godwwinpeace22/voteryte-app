          ...
          <div class="post__body">
            ...
            <v-card-actions class="pl-0">
              <div style="width:fit-content;" class="text-xs-center">
                ...
                <span class="ml-4">{{post.lc}}</span>
                <v-btn icon class="px-1" small>
                  <v-icon color="icon_btn" small>mdi-thumb-up</v-icon>
                </v-btn>

                <span class="ml-4">{{post.cc}}</span>
                <v-btn class="px-1" icon small
...
export default {
  ...
  computed: {
    ...
    uniqPosts(){
      ...
      let p = uniqBy(this.posts)
      // latest posts and posts with highest comments on top
      return orderBy(p, ['tstamp', 'cc'], ['desc', 'desc'])
    },
    postMenu(){
      return [
        {
          text: 'Expand post', 
          action: 'expandPost', 
          hidden: this.selected_post['_'] && this.selected_post['_']['#'] == this.$route.params.postId},
        {
          text: 'Copy post link', 
          action: 'copyLink', 
          divider: true
        },
        {
          text: 'Delete post', 
          action: 'deletePost',
          hidden: this.selected_post.a && this.selected_post.a.username == this.getUser.username
        },
      ]
    },
    linkify_options(){
      return {
        className: {
          hashtag: 'linkified--hashtag',
          mention: 'linkified--link'
        },
        events: {
          click: function () {
  ...
  methods: {
    ...
    deletePost(post){
      if(post.a.username == this.getUser.username){
        ...
        let postIndex = this.uniqPosts.findIndex(p => p.did == post.did)
